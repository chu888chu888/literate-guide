 ...
 * 上传util
 * @author	Chopper
 * @version	v1.0, 2012-12-24 下午2:02:47
 * @since
 */
public class UploadUtil {
	...
	//fastdfs 插件桩
	private static FastdfsBundle fastdfsBundle;
	
	/**
	 * 获取插件桩
	 * @return
	 */
	private static FastdfsBundle getFastdfsBundle(){
		if(fastdfsBundle==null){
			fastdfsBundle = SpringContextHolder.getBean("fastdfsBundle");
		}
	...
	public static String upload(File file,String fileFileName,String subFolder ) {
		...
		if (SystemSetting.getFdfs_open() == 1) { 
			String filepath = getFastdfsBundle().BeAfterUpload(new File(filePath));
			//两两对应
			getFastdfsBundle().getCache().put(filepath, filePath); 
			getFastdfsBundle().getCache().put(filePath, filepath); 
			if(filepath!=null){
				return filepath;
			}else{
