...
var OrderDetail = {
	...
	bindFlowEvent : function() {
		...
		$("#pay").bind("click",function() {
					...
					　　		closed: false,
					　　		cache: false,
					　　		href: ctx+'/shop/admin/payment/show-pay-dialog.do?ajax=yes&orderId='+ self.orderid, 	
					　　		modal: true,
					　　		buttons: [{		
					　　			 text:'保存',
					　　			 handler:function(){
		...
		$("#refund").bind("click",function() {Eop.Dialog.open("order_dialog");
			...
			if( !disabled ){
				$("#order_dialog .con").load(basePath+ "payment/show-refund-dialog.do?ajax=yes&orderId="+ self.orderid,
		...
		$("#rog").bind("click",function() {
			...
			if( !disabled ){
					if (confirm("确认顾客已收到货了吗")) {
						...
						$.ajax({url : ctx+"/shop/admin/order/rog-confirm.do?ajax=yes&orderId="+self.orderid,
							dataType : "json",
	...
	loadProductStore : function(box, itemid) {
		box.load(
				basePath + "ship/get-product-store.do?ajax=yes&itemid=" + itemid,
				function() {
	...
	refund : function() {
		var self = this;
		var options = {
			url : basePath + "payment/cancel-pay.do?ajax=yes",
			type : "post",
			dataType : "json",
			success : function(responseText) {
				if (responseText.result == 1) {
	...
	complete : function(orderId) {
		var self = this;
		$.ajax({
			url :  ctx+'/shop/admin/order/complete.do?ajax=yes&orderId=' + orderId,
			dataType : "json",
			success : function(responseText) {
				if (responseText.result == 1) {
					$.Loading.success(responseText.message);
	...
	cancel : function(orderId, canel_reason,savebtn) {
		...
		savebtn.linkbutton("disable");
		$.ajax({
			url : ctx+'/shop/admin/order/cancel.do?ajax=yes&orderId=' + orderId+ "&cancel_reason=" + canel_reason,
			dataType : "json",
			success : function(responseText) {
				if (responseText.result == 1) {
					$.Loading.success(responseText.message);
	...
	confirmOrder : function(orderId) {
		...
		$("#confirmorder").linkbutton("disable");
		$.ajax({
			url :  ctx+'/shop/admin/order/confirm-order.do?ajax=yes&orderId='+ orderId,
			dataType : "json",
			success : function(responseText) {
				if (responseText.result == 1) {
					$.Loading.success(responseText.message);
